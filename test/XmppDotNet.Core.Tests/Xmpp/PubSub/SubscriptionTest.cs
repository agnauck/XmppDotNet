...
namespace XmppDotNet.Tests.Xmpp.PubSub
{
    public class SubscriptionTest
    {
        ...
        {
            XmppXElement.LoadXml(Resource.Get("Xmpp.PubSub.subscription2.xml")).ShouldBeOfType<XmppDotNet.Xmpp.PubSub.Owner.Subscriptions>();
        }
        ...
        public void TestOwnerSubscriptions()
        {
            var ss =
                XmppXElement.LoadXml(Resource.Get("Xmpp.PubSub.subscription2.xml"))
                    .ShouldBeOfType<XmppDotNet.Xmpp.PubSub.Owner.Subscriptions>();

            Assert.Equal("princely_musings", ss.Node);
            IEnumerable<XmppDotNet.Xmpp.PubSub.Owner.Subscription> subs = ss.GetSubscriptions();
