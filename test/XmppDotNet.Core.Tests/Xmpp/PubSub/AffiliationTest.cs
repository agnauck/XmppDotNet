...
namespace XmppDotNet.Tests.Xmpp.PubSub
{
    public class AffiliationTest
    {
        ...
        {
            XmppXElement.LoadXml(Resource.Get("Xmpp.PubSub.Owner.affiliations2.xml")).ShouldBeOfType<XmppDotNet.Xmpp.PubSub.Owner.Affiliations>();
        }
        ...
        {
            var affs = new XmppDotNet.Xmpp.PubSub.Owner.Affiliations { Node = "princely_musings" };

            affs.AddAffiliation(new XmppDotNet.Xmpp.PubSub.Owner.Affiliation { Jid = "hamlet@denmark.lit", AffiliationType = AffiliationType.Owner });
            affs.AddAffiliation(new XmppDotNet.Xmpp.PubSub.Owner.Affiliation { Jid = "polonius@denmark.lit", AffiliationType = AffiliationType.Outcast });
        ...
        {
            var affs = XmppXElement.LoadXml(Resource.Get("Xmpp.PubSub.Owner.affiliations2.xml")).Cast<XmppDotNet.Xmpp.PubSub.Owner.Affiliations>();
         
            IEnumerable<XmppDotNet.Xmpp.PubSub.Owner.Affiliation> affiliations = affs.GetAffiliations();
            Assert.Equal("princely_musings", affs.Node);
