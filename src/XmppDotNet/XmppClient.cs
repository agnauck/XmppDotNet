namespace XmppDotNet
{
    ...
    public class XmppClient : XmppConnection, IXmppClient
    {
        ...
        public override async Task ConnectAsync(CancellationToken cancellationToken)
        {
            ...
            XmppSessionStateSubject.Value = XmppDotNet.SessionState.Connected;
        ...
        {
            if (XmppSessionStateSubject.Value < XmppDotNet.SessionState.Securing && features.SupportsStartTls && Tls)
            {
            ...
            else if (XmppSessionStateSubject.Value < XmppDotNet.SessionState.Registering && features.SupportsRegistration && RegistrationHandler?.RegisterNewAccount == true)
            {
            ...
            else if (XmppSessionStateSubject.Value < XmppDotNet.SessionState.Authenticating)
            {
            ...
            else if (XmppSessionStateSubject.Value < XmppDotNet.SessionState.Binding
        ...
        {
            XmppSessionStateSubject.Value = XmppDotNet.SessionState.Authenticating;
            var res = await SaslHandler.AuthenticateAsync(mechanisms, this, cancellationToken).ConfigureAwait(false);

            if (res is Success)
            {
                XmppSessionStateSubject.Value = XmppDotNet.SessionState.Authenticated;
        ...
        private async Task<Iq> DoBindAsync(StreamFeatures features, CancellationToken cancellationToken)
        {
            ...
            XmppSessionStateSubject.Value = XmppDotNet.SessionState.Binded;
        ...
        {
            XmppSessionStateSubject.Value = XmppDotNet.SessionState.Registering;
            var regInfoIqResult =
                await SendIqAsync(
            ...
            if (regInfoIqResult.Type == IqType.Result && regInfoIqResult.Query is Register)
            {
                ...
                {
                    XmppSessionStateSubject.Value = XmppDotNet.SessionState.Registered;
                    return;
                }
                else
